# signbit<a name="ZH-CN_TOPIC_0000002554344091"></a>
...
```
__simt_callee__ inline int signbit(float x)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void KernelSignbit(__gm__ int* dst, __gm__ float* x)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = signbit(x[idx]); 
}
```
