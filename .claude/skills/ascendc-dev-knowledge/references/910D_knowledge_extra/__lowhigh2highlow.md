# \_\_lowhigh2highlow<a name="ZH-CN_TOPIC_0000002523344186"></a>
...
```
__simt_callee__ inline half2 __lowhigh2highlow(const half2 x)
```

```
__simt_callee__ inline bfloat16x2_t __lowhigh2highlow(const bfloat16x2_t x)
```
...
使用bfloat16x2\_t接口需要包含"simt\_api/asc\_bf16.h"头文件，使用half2接口需要包含"simt\_api/asc\_fp16.h"头文件。
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel__lowhigh2highlow(__gm__ bfloat16x2_t* dst, __gm__ bfloat16x2_t* x)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = __lowhigh2highlow(x[idx]);
}
```
