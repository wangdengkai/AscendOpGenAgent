...
Checks performed per variant:
  1. Kernel file existence: at least one .cpp file in the kernel directory
...
def _find_kernel_files(variant_dir, op_name, mode):
    """Find kernel .cpp files in a variant directory."""
    if mode == "ops":
        patterns = [
            os.path.join(variant_dir, "modified_files", "op_kernel", "*.cpp"),
            os.path.join(variant_dir, "modified_files", "op_kernel", "**", "*.cpp"),
        ]
    else:
        ...
        patterns = [
            os.path.join(variant_dir, "kernel", "*.cpp"),
            os.path.join(variant_dir, "kernel", "**", "*.cpp"),
            os.path.join(variant_dir, f"{op_name}Custom", "op_kernel", "*.cpp"),
            os.path.join(variant_dir, f"{op_name}Custom", "op_kernel", "**", "*.cpp"),
        ]
    files = []
    for pat in patterns:
...
def _find_shared_kernels(shared_dir, op_name, mode):
    """Find baseline kernel .cpp files in shared directory."""
    if mode == "ops":
        patterns = [
            os.path.join(shared_dir, "original", "op_kernel", "*.cpp"),
            os.path.join(shared_dir, "original", "op_kernel", "**", "*.cpp"),
        ]
    else:
        # TileLang flow: kernel files in kernel/ directory
        # Also check legacy DSL path for backward compatibility
        patterns = [
            os.path.join(shared_dir, "kernel", "*.cpp"),
            os.path.join(shared_dir, "kernel", "**", "*.cpp"),
            os.path.join(shared_dir, f"{op_name}Custom", "op_kernel", "*.cpp"),
            os.path.join(shared_dir, f"{op_name}Custom", "op_kernel", "**", "*.cpp"),
        ]
    files = []
    for pat in patterns:
