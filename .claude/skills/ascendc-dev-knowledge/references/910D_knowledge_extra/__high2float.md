# \_\_high2float<a name="ZH-CN_TOPIC_0000002554423805"></a>
...
```
__simt_callee__ inline float __high2float(const half2 x)
```

```
__simt_callee__ inline float __high2float(const bfloat16x2_t x)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel__high2float(__gm__ float* dst, __gm__ bfloat16x2_t* x)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = __high2float(x[idx]);
}
```
