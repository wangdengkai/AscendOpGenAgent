# \_\_float22half2\_rn<a name="ZH-CN_TOPIC_0000002554344117"></a>
...
```
__simt_callee__ inline half2 __float22half2_rn(const float2 x)
```
...
将输入的两个分量遵循CAST\_RINT模式转换成的half2类型数据。
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel__float22half2_rn(__gm__ half2* dst, __gm__ float2* x)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = __float22half2_rn(x[idx]);
}
```
