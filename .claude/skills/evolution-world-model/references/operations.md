...
本文件定义了 ops-evo Agent 在进化过程中对 `world_model.json` 执行的四种核心操作。
...
- 基线评测结果（Mode B 专有）
- `.claude/skills/evolution-strategies/references/strategy-index.md`
- `parallel_num` 参数
...
```bash
python3 .claude/skills/evolution-world-model/scripts/wm_ops.py validate --path {world_model_path}
```
...
也可手动执行：
```bash
python3 .claude/skills/evolution-world-model/scripts/wm_ops.py prune --path {world_model_path}
```
...
Round 1:
  ...
  [并行生成] → ops-partial × parallel_num（每个变体有明确策略）
  [收集结果]
