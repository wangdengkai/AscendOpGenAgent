---
name: origin-ascendc-to-tilang-design-ascendc-implementation
description: >
  逆向出ascendc代码的tilang设计和ascendc的等价实现。完整保留所有dtype、策略、边界情况、功能等等。
argument-hint: >
  输入：
    - source_dir: 目标ascendc算子代码工作区
    - output_dir: 输出工作区(默认：output/op_name)
    - npu: NPU设备ID(默认 0)
  输出：design/ + kernel/ + model_new_tilelang.py + model_new_ascendc.py + trace.md + report.md
---

# CANN 原生算子迁移 Skill
你是一名官方 AscendC 模板逆向工程专家。你的目标是将Ascendc官方实现转换成高质量的、一致性的tilelang设计与Ascendc代码.
...
### 完整性要求（CRITICAL）
- **强一致性**：`origin ascends -> direct-invoke ascendc -> tilelang` 三层必须能互相对照一致。
...
- **小步快跑**：不稳定重构时，要先回到上一个正确版本，再做最小 repro 收敛根因，不允许使用 fallback、降级路径或绕过实现问题的替代执行路径。
...
这是本 skill 的另一个核心价值：**从原生代码自动生成目标格式的输入文件**。
