...
- **策略选择协议（分层检索）**: 读取 `.claude/skills/evolution-strategies/references/strategy-index.md` 后，按以下顺序选择策略：
...
- **知识库查询协议**: `.claude/skills/evolution-knowledge/references/a3/` 采用渐进式披露，先读 guide.md 快速参考，按需深入。
  **Init 阶段必读**:
...
- **重入会话**（崩溃恢复 / context compression 后重入）：**第一件事**就是读 state.json：
  ```bash
  python3 .claude/skills/evolution-world-model/scripts/state_ops.py read --evo-dir "$EVO_DIR"
  ```
...
- **Stop hook 阻塞**：本仓配置了 `.claude/hooks/loop-stop.sh`（Stop 事件触发时校验 state.json 与产物一致性）。若 hook 报 `R2.x / R3 / R4 / R5` block，**不要**通过设置 `LOOP_HOOK_DISABLE=1` 绕过，应当修复 state（如补跑必跑步骤、完成未完结的 partial）。
...
- **目标加速比**: 相对于baseline的目标提升倍数（如 1.5x 表示比baseline快50%）
  - **multi-shape 模式下**: 该值作为**硬门控阈值**，每个 target shape 必须 ≥ 此值才算"目标达成"
...
- **generalization_shapes** (可选): 泛化保护 shape 列表（schema 同 target_shapes），仅在某变体达成 fully_passed 时跑一次做兜底验证；几何平均 ≥ 1.0x 即通过
  - 不提供时跳过泛化验证（fully_passed 即视为最终达成）
- **停滞窗口** (可选): 连续多少轮无显著提升后提前终止（默认: 自动计算）
- **设备ID** (可选): NPU设备ID（默认: 0）

> **shape 命名约束**：每个 shape 的 `name` 字段必须在 target_shapes / generalization_shapes 全集内**唯一**，建议短 id 风格（如 `T1` / `G1`），避免类似 `npn_2_4096_5120_fp16` 这种冗长名（会影响进度展示和报告可读性）。重名时 evaluate 入口直接 raise。
...
```bash
python3 .claude/skills/evolution-world-model/scripts/session_anchor.py write \
    --op-name {op_name} \
    --evo-dir "$EVO_DIR" \
...
| 顶层字段 | 必填 | 说明 |
|---------|------|------|
| `op_namespace` / `op_func` | 是 | 标准仓 `npu` / `npu_{op_name}`；omni-ops `custom` / `npu_{op_name}` |
| `target_shapes[]` | 是 | 每项含 `name`（短 id 全局唯一）+ `inputs[]` + `scalar_args{}` |
| `generalization_shapes[]` | 否 | schema 同 target_shapes；不提供则跳过泛化兜底 |
| `output_count` | 是 | 输出数量 |
| input 项结构 | — | `{name, shape: list[int], dtype: str}` |

最小示例：

```json
{ "op_namespace": "npu", "op_func": "npu_xxx", "output_count": 1,
  "target_shapes": [
    {"name": "T1",
     "inputs": [{"name": "x", "shape": [2, 4096, 5120], "dtype": "float16"}],
     "scalar_args": {"epsilon": 1e-5}}
  ],
  "generalization_shapes": [] }
```

**向后兼容**：旧 call_spec（顶层直接含 `inputs` / `scalar_args`）由 `evaluate_ops_direct.py` 的 `normalize_call_spec` 自动包装为 `target_shapes=[{"name":"default", ...}]`、`generalization_shapes=[]`，无需手工迁移。
...
> **multi-shape 字段**：`baseline_evaluation.json` 含 `shape_results.target` / `shape_results.generalization` per-shape 时间；自评模式下所有 `speedup=1.0`。后续变体评估通过 `--baseline-cache baseline_evaluation.json` 复用 per-shape baseline 时间。
> **向后兼容**：单 shape call_spec（顶层 `inputs`）会被 normalize_call_spec 自动包装成 `target_shapes=[{"name":"default",...}]`，`shape_results.target` 长度为 1，`shape_results.generalization` 为空。

baseline_evaluation.json 中的 `shape_results.target[*].baseline_time_us`（或旧 `baseline.time_us`）作为后续对比的参考基准。
...
> **关键校验 — 精度失败立即终止**: 读取 `baseline_evaluation.json`，确认 baseline 的 `compilation_success` 与 `precision_passed` **全部** 为 true。
> - 字段路径：优先 `shape_results.target[*]`（multi-shape），不存在则回退 legacy `baseline.{...}`
> - 任一字段为 false → 设 `should_continue = false`，立即终止进化并输出诊断
> - `precision_passed=false` 时，优先检查步骤 3.3 生成的 `call_spec.json` 是否与算子实际接口一致；若算子目录下存在 benchmark 脚本，应优先复用其测试参数
...
2. **必读知识库**: 读取 `.claude/skills/evolution-knowledge/references/a3/hardware/guide.md` 和 `optimization_patterns/guide.md`
...
4. 读取 `.claude/skills/evolution-strategies/references/strategy-index.md`，识别最相关策略
...
6. **写入 session 身份锚定到 world_model.json**（必须在写入 world_model.json 时执行）：
   ```bash
   python3 .claude/skills/evolution-world-model/scripts/wm_ops.py session \
...
9. **（必须执行）** 挂载 baseline profiling 证据到根级 `baseline_evidence` 字段。这是后续 SELECT 的 baseline 对齐惩罚（`w_baseline_mismatch`）和 partial-agent prompt 的 Baseline 行注入的数据源：
   ```bash
   python3 .claude/skills/evolution-world-model/scripts/wm_ops.py attach-baseline-evidence \
       --wm-path "$EVO_DIR/world_model.json" \
...
**世界模型JSON格式**: 参考 `.claude/skills/evolution-world-model/references/schema.md`
...
```bash
python3 .claude/skills/evolution-world-model/scripts/state_ops.py init \
    --evo-dir "$EVO_DIR" \
    --agent ops-evo \
    --session-id "{op_name}_ops-evo_${TIMESTAMP}" \
...
```bash
python3 .claude/skills/evolution-world-model/scripts/session_anchor.py read --op-name {op_name}
```
...
- 每轮 refine 后更新 session anchor 的 `actual_rounds_completed`：
  ```bash
  python3 .claude/skills/evolution-world-model/scripts/session_anchor.py update \
      --op-name {op_name} \
      --actual-rounds $r
  ```
...
若 `world_model_active = true`:
  ...
  - **检查B — 停滞/斜率检测（Supervisor Agent 介入）**:
    ...
    **若可介入（触发条件成立 且 通过冷却 和 硬上限）**：
    ...
    准备 Supervisor 输入信息：
    - 运行 `python3 .claude/skills/evolution-world-model/scripts/wm_ops.py summary --path {world_model_path}` 获取世界模型概览
...
读取 `.claude/skills/evolution-strategies/references/prompts/supervisor-prompt.md` 获取完整模板，填充 `{变量}` 后作为 prompt 启动 Supervisor Agent。
...
- 必须读 .claude/skills/evolution-knowledge/references/a3/proven_solutions/INDEX.md 至少 3 个未在前几轮 inspirations 中出现过的条目
...
参考 `.claude/skills/evolution-world-model/references/operations.md` 中的 **操作二：Select**。

**若 `world_model_active = true`**:

0. **精简读取**：先执行 `python3 .claude/skills/evolution-world-model/scripts/wm_ops.py summary --path {world_model_path}` 获取概览，仅阅读 summary 输出来理解当前状态。只在需要修改节点字段时才读取完整 `world_model.json`。

1. **脚本化选择（必须执行，保证分支多样性约束生效）**：

```bash
SELECTIONS=$(python3 .claude/skills/evolution-world-model/scripts/wm_ops.py select \
...
读取 `.claude/skills/evolution-strategies/references/prompts/ops-partial-prompt.md` 获取完整 prompt 模板，按下方变量填充规则填充后启动子 agent。
...
- `{shapes_mode}`:
  - `"target"`：当节点 `strategy_combination` 含 `"P-ShapeSpec-01"`（改动隔离在 specialized variant，default 未动 → 进化期跳过泛化以省时间，最终 fully_passed 时 §4.6.5 再补跑）
  - `"all"`：当节点 `strategy_combination` 不含 `"P-ShapeSpec-01"`（变体可能改了 default 分支 → 每轮都跑泛化让退化立即暴露）
- `{target_speedup}`：用户在 §1 提供的 target_speedup 值，传给 evaluate_ops_direct.py 的 `--target-speedup`，用于判定 `all_target_meet_target` / `gating`

**v3.2 Strategy Resources 变量填充**（C7 新增，必须执行）:

`{strategy_resources_block}` 由 helper 脚本一次性生成（封装 Preconditions 过滤 + Playbook 加载 + Excluded 段拼装）。**每个 partial 启动前**，主 agent 必须为该节点执行：
...
- 脚本自动：
  1. 调 `wm_ops filter-candidates` 用 Preconditions YAML 过滤候选，写入 `node.filtered_by`
  2. 调 `load_playbook.py` 加载通过的策略的 Playbook 全文（仅 has_playbook=true 的）
  3. 调 `state_ops get-read-keys` 拿 Excluded 段
  4. 拼接成完整 markdown 块写入 `/tmp/srblock_{node_id}.md`
- 退出码：
  - `0` success → 把 `cat /tmp/srblock_{node_id}.md` 内容填入 `{strategy_resources_block}` 变量
  - `2` 全部候选被过滤 → 该 partial 退回 `open_exploration` 模式（在 `{node_description}` 中明确切换），`{strategy_resources_block}` 填空字符串
  - `1` 脚本本身失败 → fail-safe: `{strategy_resources_block}` 填空字符串，partial 自动 fall back 到老路径（读 .claude/skills/evolution-strategies/references/cards/{ID}_*.md）
- Fallback 容错：若 prepare_strategy_resources.py 不存在（旧版本 worktree），`{strategy_resources_block}` 填空字符串。

**v3.2 Refine 后追加 read_keys**（C7 新增，在 4.4.1b 之后、4.4.1c 之前执行）:

每轮 refine 写完 lineage.jsonl 后，把本轮所有 passed 节点采纳的 source_keys 加入 do-not-reread 集合，下一轮 partial 自动跳过：
...
KEYS=$(jq -r '.source_keys[]' \
    ...
    | sort -u | paste -sd,)
if [[ -n "$KEYS" ]]; then
    python3 .claude/skills/evolution-world-model/scripts/state_ops.py add-read-keys \
        --evo-dir "$(pwd)/output/{op_name}_ops-evo_{timestamp}/" \
        --keys "$KEYS"
fi
```

drift_status=replan_required 时，主 agent 应在 SELECT 前调 `state_ops clear-read-keys`
强制扩搜索（与 wm_ops drift breaker 配套，让上轮已读源回到 candidate 池）。
...
```bash
python3 .claude/skills/evolution-world-model/scripts/wm_ops.py refine \
...
`wm_ops.py refine` 自动在每个 passed 节点写入 `node.facts`（Stage 1 纯事实抽取），但 `node.diagnosis` 字段需要 LLM 在此步基于 facts + 历史对比 + cannbot quickref 补充。
...
2. 对每个 passed 节点的 `facts`，参考 `.claude/skills/evolution-knowledge/references/a3/profiling_reference/optimization_quickref.md` 和 `INDEX.md`（含 18 项标准 `bottleneck_labels` 词表），给出**结构化诊断**：

   ```yaml
   # 必填四字段
   diagnosis_text: |
     <自由文本，≥ 20 字符，描述你看到的瓶颈和根因假设>
     例：MTE2 busy 85% 但带宽利用 32%，判定为假 MTE2 bound，
         根因是 scale 小块重复 DMA 搬运
   bottleneck_labels:                     # 必须从 18 项词表选
     - fake_mte2_bound                    # ≤ 3 个 label，按重要性降序
     - undersize_transfer
   confidence: 0.75                       # 0~1 float，对诊断的置信度
...
4. 写完后**必须**跑校验：
   ```bash
   python3 .claude/skills/evolution-world-model/scripts/wm_ops.py validate-diagnosis \
     --wm-path "output/{op_name}_ops-evo_{timestamp}/world_model.json" \
     --strict
   ```
   若 exit code = 2，根据输出修正后重做。

5. **必须**调 `finalize-ledger` 把刚写入的 diagnosis 回填到 `artifacts/lineage.jsonl` 与 `attempt-ledger.md`，**同时把 `next_round_hint` 应用到 open 子节点的 `strategy_combination`**（refine 派生时 LLM diagnosis 尚未存在，hint 必须在此处迟到生效）：
   ```bash
   python3 .claude/skills/evolution-world-model/scripts/wm_ops.py finalize-ledger \
     --wm-path "output/{op_name}_ops-evo_{timestamp}/world_model.json" \
     --evo-dir   "output/{op_name}_ops-evo_{timestamp}"
   ```
   该命令幂等，可安全多次执行。否则：(a) 跨 session 的 `discover_strategies.py` 提炼会丢失诊断信息；(b) 下一轮派生的 open 子节点 `strategy_combination` 不会反映 hint.prefer/avoid，hint 形同虚设。

**何时跳过本步骤**：
- 若 `node.facts` 为 null（极少数情况，profiling 完全缺失）→ 该节点 diagnosis 留空
- 若 `node.status != "passed"`（编译/精度失败节点不需要诊断）
...
```bash
# 对每个诊断结果，调用脚本写入
python3 .claude/skills/evolution-world-model/scripts/wm_ops.py diagnose \
    --wm-path "output/{op_name}_ops-evo_{timestamp}/world_model.json" \
    --node-id {node_id} \
...
3. 读取 `.claude/skills/evolution-strategies/references/strategy-index.md`，判断新颖性：
   该优化手段是否超出现有所有策略的范畴？（不是组合，而是一种新方法论）
4. **若确认新颖**：
   a. 列出 `.claude/skills/evolution-strategies/references/discovered/disc_X*.md` 文件，确定下一个 X 编号（如 X1、X2…）
   b. 写入新策略文件 `.claude/skills/evolution-strategies/references/discovered/disc_X{n}.md`，格式：
      ```markdown
      ---
      id: X{n}
      origin: discovered
...
[multi-shape 模式] Per-Shape 表（每个变体一行）:
  variant | gating         | T1     | T2     | min   | max   | parent_eligible
  --------+----------------+--------+--------+-------+-------+----------------
  p0/n1   | partial_passed | 1.40x  | 1.05x  | 1.05x | 1.40x | true
  p1/n2   | target_regression | 1.50x | 0.85x | 0.85x | 1.50x | false  ← 屏蔽
  p2/n3   | fully_passed   | 1.30x  | 1.25x  | 1.25x | 1.30x | true  ← 候选
...
**单 shape 模式**:
- **目标达成**: 任意 variant 的加速比 ≥ target_speedup → 设置 `should_continue = false`，进入步骤5→6
- **全部失败**: 本轮所有 variant 均失败 → 设置 `should_continue = false`，进入步骤5→6
- **否则**: `r += 1`，返回步骤4.1 GATE
...
**4.6.5 fully_passed 候选泛化验证**

**触发条件**: 本轮存在节点 `gating == "fully_passed"`。
...
2. **合并结果到 world_model**:
   ```python
   gen_eval = load_json(".../generalization_eval.json")
   node.shape_results.generalization = gen_eval.shape_results.generalization
   node.aggregate.generalization_geo_mean_speedup = gen_eval.aggregate.generalization_geo_mean_speedup
   node.aggregate.any_generalization_regression = gen_eval.aggregate.any_generalization_regression
   ```

3. **判定 + 分流**:

   **若 `any_generalization_regression == false`**（泛化通过）:
   - 视为最终达成 → 设置 `should_continue = false`，进入步骤5→6
   - 展示成功报告（每个 shape 的 speedup）

   **若 `any_generalization_regression == true`**（泛化退化）:
   - 节点 `gating = "generalization_regression"`，`parent_eligible = false`
   - **挂起本轮进化，调用 supervisor 决策**（不静默降级）：
     - Supervisor 输入：fully_passed 候选的 per-target speedup + 退化的泛化 shape 列表 + 该节点的 strategy_combination
     - Supervisor 输出枚举：
...
python3 .claude/skills/evolution-world-model/scripts/session_anchor.py verify \
    --op-name {op_name} \
    --evo-dir "$EVO_DIR"

# 2. 交叉校验 world_model.json 中的 session 字段
python3 .claude/skills/evolution-world-model/scripts/wm_ops.py session-verify \
    --wm-path "$EVO_DIR/world_model.json" \
    --evo-dir "$EVO_DIR"
```
...
EVO_DIR=$(python3 .claude/skills/evolution-world-model/scripts/session_anchor.py read --op-name {op_name} | python3 -c "import sys,json; print(json.load(sys.stdin)['evo_dir'])")
...
世界模型格式参考 `.claude/skills/evolution-world-model/references/schema.md` 和 `operations.md`。
