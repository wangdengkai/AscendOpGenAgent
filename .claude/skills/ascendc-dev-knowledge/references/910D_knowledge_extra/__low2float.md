# \_\_low2float<a name="ZH-CN_TOPIC_0000002523343906"></a>
...
```
__simt_callee__ inline float __low2float(const half2 x)
```
...
```
__simt_vf__ __launch_bounds__(1024) inline void kernel__low2float(__gm__ float2* dst, __gm__ bfloat16x2_t* x)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    dst[idx] = __low2float(x[idx]);
}
```
