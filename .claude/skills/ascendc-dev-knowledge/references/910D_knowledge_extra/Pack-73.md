# Pack<a name="ZH-CN_TOPIC_0000002523304908"></a>
...
__simd_vf__ inline void PackVF(__ubuf__ T* dstAddr, __ubuf__ U* srcAddr, uint32_t oneDstRepSize, uint32_t oneSrcRepSize, uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        ...
        if constexpr (mode == 0) {
            AscendC::MicroAPI::Pack<T, U, AscendC::MicroAPI::HighLowPart::LOWEST>(dstReg, srcReg);
        } else if constexpr (mode == 1) {
            AscendC::MicroAPI::Pack<T, U, AscendC::MicroAPI::HighLowPart::HIGHEST>(dstReg, srcReg);
        }
