...
用于 lingxi-evo 进化优化过程中的设备侧精确性能评测：

| 脚本 | 说明 |
|------|------|
| `lingxi_perf_driver.py` | Profiler 驱动：通过 torch_npu.profiler 解析 kernel_details.csv，获取设备侧 kernel 精确时延。每个 case × 每种实现使用独立子进程。 |
| `lingxi_perf_single.py` | 单 case worker：被 lingxi_perf_driver.py 通过子进程调用，输出 JSON 到 stdout。 |
| `lingxi_perf_bench.py` | Device-side event timing：使用 torch.npu.Event 替代 host wall-clock，含 IQR 异常值剔除。 |
| `lingxi_eval_wrapper.py` | 一条龙封装：build_ascendc.py + verification_ascendc.py + lingxi_perf_driver.py，输出结构化 JSON。 |
